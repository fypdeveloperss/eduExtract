...
const PreferencesSettings = ({ embedded = false }) => {
  const { preferences, updatePreferences, resetToDefaults, fetchPreferences } =
    usePreferences();
  ...
  useEffect(() => {
    ...
    const p = preferences || {};
    console.log("Preferences from context:", JSON.stringify(p, null, 2));

    const merged = {
      ...defaultPrefs,
      ...p,
      contentPreferences: {
        ...defaultPrefs.contentPreferences,
        ...(p.contentPreferences || {}),
      },
      tonePreferences: {
        ...defaultPrefs.tonePreferences,
        ...(p.tonePreferences || {}),
      },
      learningBehavior: {
        ...defaultPrefs.learningBehavior,
        ...(p.learningBehavior || {}),
      },
      contentCustomization: {
        ...defaultPrefs.contentCustomization,
        ...(p.contentCustomization || {}),
      },
      studyProfile: {
        ...defaultPrefs.studyProfile,
        ...(p.studyProfile || {}),
        // Preserve actual values even if they're empty strings
        purpose:
          p.studyProfile?.purpose !== undefined
            ? p.studyProfile.purpose
            : defaultPrefs.studyProfile.purpose,
        school:
          p.studyProfile?.school !== undefined
            ? p.studyProfile.school
            : defaultPrefs.studyProfile.school,
        courses:
          p.studyProfile?.courses !== undefined
            ? p.studyProfile.courses
            : defaultPrefs.studyProfile.courses,
      },
    };
    ...
    if (!coursesInputTouched.current) {
      const coursesString = (merged.studyProfile.courses || []).join(", ");
      console.log("Setting coursesInput to:", coursesString);
      setCoursesInput(coursesString);
    }
  }, [preferences]);
  ...
  const handleSave = async () => {
    try {
      ...
      const savedPreferences = await updatePreferences(payload);
      console.log(
        "Saved preferences returned:",
        JSON.stringify(savedPreferences, null, 2),
      );

      // Reset coursesInputTouched so courses input can be updated from saved preferences
      coursesInputTouched.current = false;

      // Force refresh preferences from backend to ensure we have the latest data
      ...
      {!embedded && (
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleReset}
              className="px-3 py-1.5 bg-gray-100 dark:bg-[#fafafa1a] text-gray-700 dark:text-[#fafafacc] rounded-lg hover:bg-gray-200 dark:hover:bg-[#fafafa2a] transition-colors flex items-center gap-2"
            >
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-3 py-1.5 bg-[#171717] dark:bg-[#fafafa] text-white dark:text-[#171717] rounded-lg hover:bg-[#333333] dark:hover:bg-[#e5e5e5] disabled:opacity-50 transition-colors flex items-center gap-2"
            >
              {saving ? (
      ...
      {embedded && (
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleReset}
              className="px-3 py-1.5 text-sm bg-gray-100 dark:bg-[#fafafa1a] text-gray-700 dark:text-[#fafafacc] rounded-lg hover:bg-gray-200 dark:hover:bg-[#fafafa2a] transition-colors flex items-center gap-2"
            >
              <RotateCcw className="w-4 h-4" />
              <span>Reset</span>
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-3 py-1.5 text-sm bg-[#171717] dark:bg-[#fafafa] text-white dark:text-[#171717] rounded-lg hover:bg-[#333333] dark:hover:bg-[#e5e5e5] disabled:opacity-50 transition-colors flex items-center gap-2"
            >
              {saving ? (
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-current"></div>
              ) : (
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-[#fafafa1a] rounded-lg">
            <div>
              <h3 className="font-medium text-gray-900 dark:text-[#fafafa]">
                Include Examples
              </h3>
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-[#fafafa1a] rounded-lg">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-[#fafafacc]">
                Use user's field/industry examples
              </p>
            </div>
            <input
              type="checkbox"
              checked={
                localPreferences.contentCustomization.personalizedExamples
              }
              onChange={(e) =>
                setLocalPreferences({
                  ...localPreferences,
